...
class Square(Attack):
    ...
    def forward(self, images, timestep, labels):
        ...
        adv_images = self.perturb(images, timestep, labels)

        return adv_images

    def margin_and_loss(self, x, timestep, y):
        """
        :param y:        correct labels if untargeted else target labels
        """

        logits = self.get_logits(x, timestep)
    ...
    def attack_single_run(self, x, timestep, y):
        with torch.no_grad():
            adv = x.clone()
            c, h, w = x.shape[1:]
            n_features = c * h * w
            ...
            if self.norm == "Linf":
                ...
                margin_min, loss_min = self.margin_and_loss(x_best, timestep, y)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    margin, loss = self.margin_and_loss(x_new, timestep, y_curr)
    ...
    def perturb(self, x, timestep, y=None):
        """
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                _, adv_curr = self.attack_single_run(x_to_fool, timestep, y_to_fool)

                output_curr = self.get_logits(adv_curr, timestep)
                if not self.targeted:
