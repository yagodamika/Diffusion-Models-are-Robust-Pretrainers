...
class FAB(Attack):
    ...
    def forward(self, images, t, labels):
        r"""
        Overridden.
        """
        ...
        adv_images = self.perturb(images, t, labels)

        return adv_images

    def _get_predicted_label(self, x, timestep):
        with torch.no_grad():
            outputs = self.get_logits(x, timestep)
    ...
    def get_diff_logits_grads_batch_targeted(self, imgs, timestep, la, la_target):
        u = torch.arange(imgs.shape[0])
        im = imgs.clone().requires_grad_()
        with torch.enable_grad():
            y = self.get_logits(im, timestep)
    ...
    def attack_single_run_targeted(self, x, timestep, y=None, use_rand_start=False):
        """
        :param x:    clean images
        :param y:    clean labels, if None we use the predicted labels
        """
        ...
        y_pred = self._get_predicted_label(x, timestep)
        if y is None:
            y = y_pred.detach().clone().long().to(self.device)
        else:
            y = y.detach().clone().long().to(self.device)
        ...
        output = self.get_logits(x, timestep)
        ...
        while counter_restarts < 1:
            ...
            while counter_iter < self.steps:
                with torch.no_grad():
                    ...
                    is_adv = self._get_predicted_label(x1,timestep) != la2
    ...
    def perturb(self, x, timestep, y):
        adv = x.clone()
        with torch.no_grad():
            acc = self.get_logits(x, timestep).max(1)[1] == y
            ...
            def inner_perturb(targeted, timestep):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        if targeted:
                            adv_curr = self.attack_single_run_targeted(
                                x_to_fool, timestep, y_to_fool, use_rand_start=(counter > 0)
                            )
                        else:
                        ...
                        acc_curr = self.get_logits(adv_curr, timestep).max(1)[1] == y_to_fool
                        if self.norm == "Linf":
                            res = (
                                (x_to_fool - adv_curr)
                                .abs()
            ...
            if self.multi_targeted:
                for target_class in range(2, self.n_target_classes + 2):
                    ...
                    inner_perturb(True, timestep)
            elif self.targeted:
                self.target_class = self.get_target_label(x, y)
                inner_perturb(True, timestep)
            else:
                inner_perturb(False, timestep)
        return adv
